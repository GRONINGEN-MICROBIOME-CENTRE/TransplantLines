...
CDmeta = read.table("mock_data/MOCK_Meta_cleaned_v2.csv",sep=',',header=T)
CDmeta$ID <- as.character(CDmeta$ID)
CDmeta$ID <- trimws(CDmeta$ID)
rownames(CDmeta) <- CDmeta$ID

centroids_fig2a <- 
  read.csv("mock_data/CDtaxa_All_sample_loadings.csv", row.names=1) %>% 
...
# Distance to HC along PC1 and PC2
