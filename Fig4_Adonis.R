...
CDpwys <- read.table("mock_data/Mocks_Pathways_Filtered.txt",sep='\t',header=T)
CDpwys$ID <- gsub('MOCK_','',trimws(as.character(CDpwys$ID)))
rownames(CDpwys) <- CDpwys$ID

CDcard <- read.table("mock_data/Mocks_CARD_Filtered.txt",sep='\t',header=T)
CDcard$ID <- gsub('MOCK_','',trimws(as.character(CDcard$ID)))
CDcard <- CDcard[!duplicated(CDcard$ID),]
rownames(CDcard) <- CDcard$ID 

CDvfdb <- read.table("mock_data/Mocks_VF_Filtered.txt",sep='\t',header=T)
CDvfdb$ID <- gsub('MOCK_','',trimws(as.character(CDvfdb$ID)))
CDvfdb <- CDvfdb[!duplicated(CDvfdb$ID),]
rownames(CDvfdb) <- CDvfdb$ID 

CDmeta <- read.table('mock_data/MOCK_Meta_cleaned_v2.sav',sep=',',header=T)
...
plot(g)
ggsave(plot = g,filename = "mock_data_results/MockData_Fig4_ADONIS_results_LTx.png")
print(adonis_meta[order(adonis_meta$`FDR(BH)`),])
write.table(adonis_meta,"mock_data_results/MockData_Fig4_ADONIS_results_LTx.csv",sep=',',row.names = F)
...
rownames(ADONIS_Within_NTxOld_Taxa) <- ADONIS_Within_NTxOld_Taxa$ID.x
ADONIS_Within_NTxOld_Taxa$ID <- ADONIS_Within_NTxOld_Taxa$ID.x; 
ADONIS_Within_NTxOld_Taxa$ID.x <- NULL; ADONIS_Within_NTxOld_Taxa$ID.y <- NULL
...
g <- ggplot(adonis_meta, aes(reorder(row.names(adonis_meta), R2), R2, fill=Significant)) + 
  ...
  ylab ("Explained variance (R^2) ") + xlab ("Factor")  + theme(text = element_text(size=11))
plot(g)
ggsave(plot = g,filename = "mock_data_results/MockData_Fig4_ADONIS_results_NTX.png")
write.table(adonis_meta,"MockData_Fig4_ADONIS_results_NTX.csv",sep=',',row.names = F)