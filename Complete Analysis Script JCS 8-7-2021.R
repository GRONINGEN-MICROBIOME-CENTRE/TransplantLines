...
install.packages(c('foreign','tidyverse','ggplot2','ggsignif','ggpubr','survival','rms','xlsx','survminer'))
...
library(survminer)
#2A Ordination CS
#CDmeta = read.spss("mock_data/MOCK_Meta_cleaned.", to.data.frame=TRUE)
CDmeta <- read.table('mock_data/MOCK_Meta_cleaned.sav',sep=',',header=T)
...
CDmeta_All <- subset(CDmeta, Cross_sectional_samples != "No")
...
centroids_fig2a <- 
  ...
  select(ID, PC1, PC2) %>% 
  filter(ID %in% CDmeta_All[CDmeta_All$TypeMoreThan12MonthsDag3 %in% c("LTR","RTR","Healthy Control"),]$ID) %>%
  left_join(select(CDmeta_All, ID, TypeMoreThan12MonthsDag3), by="ID") %>%
  mutate(TypeMoreThan12MonthsDag3=factor(TypeMoreThan12MonthsDag3, levels=c("LTR","RTR","Healthy Control"))) %>% 
  group_by(TypeMoreThan12MonthsDag3) %>% 
...
Fig2A <- 
  ...
  select(ID, PC1, PC2) %>% 
  filter(ID %in% CDmeta_All[CDmeta_All$TypeMoreThan12MonthsDag3 %in% c("LTR","RTR","Healthy Control"),]$ID) %>%
  left_join(select(CDmeta_All, ID, TypeMoreThan12MonthsDag3), "ID") %>%
  mutate(TypeMoreThan12MonthsDag3=factor(TypeMoreThan12MonthsDag3, levels=c("LTR","RTR","Healthy Control"))) %>% 
  ggplot(aes(x=PC1, y=PC2, color=TypeMoreThan12MonthsDag3)) +
  ...
  geom_point(data=centroids_fig2a, aes(PC1_mean, PC2_mean, fill=TypeMoreThan12MonthsDag3), shape=21, size=6, color="black") +
...
TaxaLoadings <- subset(TaxaLoadings, TypeMoreThan12MonthsDag3=="RTR"|TypeMoreThan12MonthsDag3=="LTR"|TypeMoreThan12MonthsDag3=="Healthy Control")
summary(TaxaLoadings$TypeMoreThan12MonthsDag3)
...
CDmetaOud <- subset(CDmetaOud, TypeMoreThan12MonthsDag3 !="NA")
CDmetaOud$TypeMoreThan12MonthsDag3 <- droplevels(CDmetaOud$TypeMoreThan12MonthsDag3)
CDmetaOud$TypeMoreThan12MonthsDag3 <- recode_factor(CDmetaOud$TypeMoreThan12MonthsDag3, "Healthy Control" = "HC")
summary(CDmetaOud$TypeMoreThan12MonthsDag3)

CDmetaOud$TypeMoreThan12MonthsDag3_v2 <- fct_relevel(CDmetaOud$TypeMoreThan12MonthsDag3, "HC", "LTR", "RTR")
summary(CDmetaOud$TypeMoreThan12MonthsDag3_v2)

CDmetaOudRTR <- subset(CDmetaOud, TypeMoreThan12MonthsDag3=="LTR"|TypeMoreThan12MonthsDag3=="HC")
wilcox <-wilcox.test(Shannon_Species ~ TypeMoreThan12MonthsDag3, data=CDmetaOudRTR)
wilcox$p.value

Fig2B <- ggplot(CDmetaOud, aes(x=TypeMoreThan12MonthsDag3_v2, y=Shannon_Species, fill=TypeMoreThan12MonthsDag3_v2)) +
  geom_jitter(aes(color=TypeMoreThan12MonthsDag3_v2), alpha=0.0, width=0.1)+
  geom_boxplot(aes(color=TypeMoreThan12MonthsDag3_v2), alpha=0, width=0.3, size=0.8)+
  theme_classic()+
  theme(
  ...
  geom_signif(comparisons = list(c("HC", "RTR"),
              ...
              annotations = c("6.3 x 10-21",
                              "3.1 x 10-9",
                              "0.3"))+
  geom_violin(aes(color = TypeMoreThan12MonthsDag3_v2), trim = FALSE, position = position_dodge(0.9), alpha=0, size=0.8)+
...
Surv<- coxph(formula = Surv(MortalityDissimilarity$Days_since_sampling, MortalityDissimilarity$DeathNumeric) 
             ~ scale(MortalityDissimilarity$DToHC) + 
               ...
               MortalityDissimilarity$ALGLEEFT +
               MortalityDissimilarity$GESLACHT)
summary(Surv)
...
CDmetaSurvival_RTR$GESLACHT=as.character(CDmetaSurvival_RTR$GESLACHT)
...
CDmetaSurvival_RTR$ALGLEEFT=as.numeric(CDmetaSurvival_RTR$ALGLEEFT)
...
Surv<- coxph(formula = Surv(CDmetaSurvival_RTR$Days_since_sampling, CDmetaSurvival_RTR$DeathNumeric) 
             ~ CDmetaSurvival_RTR$X + 
               ...
               CDmetaSurvival_RTR$ALGLEEFT +
               CDmetaSurvival_RTR$GESLACHT)
summary(Surv)
...
meta_liver <- CDmeta_All %>%
  select(ID, TypeMoreThan12MonthsDag3, ALGLEEFT, BMI, GESLACHT, trans_roke_1, dum_ppi, dum_laxative, dum_antibiotics) %>%
  filter(TypeMoreThan12MonthsDag3 %in% c("Healthy Control","LTR")) %>%
  mutate(Group=relevel(factor(TypeMoreThan12MonthsDag3), ref="Healthy Control")) %>%
  select(-TypeMoreThan12MonthsDag3)

summary(meta_liver$Group)
sum(is.na(meta_liver$ALGLEEFT))
sum(is.na(meta_liver$BMI))
sum(is.na(meta_liver$GESLACHT))
sum(is.na(meta_liver$trans_roke_1))
summary(meta_liver$trans_roke_1)
sum(is.na(meta_liver$dum_ppi))
sum(is.na(meta_liver$dum_laxative))
sum(is.na(meta_liver$dum_antibiotics))

with(meta_liver, tapply(trans_roke_1, Group, summary))
with(meta_liver, tapply(dum_ppi, Group, summary))
with(meta_liver, tapply(dum_laxative, Group, summary))
with(meta_liver, tapply(dum_antibiotics, Group, summary))

wilcox.test(ALGLEEFT ~ Group, data=meta_liver)
wilcox.test(BMI ~ Group, data=meta_liver)
with(meta_liver, chisq.test(GESLACHT, Group))
with(meta_liver, chisq.test(trans_roke_1, Group))
with(meta_liver, chisq.test(dum_ppi, Group))
with(meta_liver, chisq.test(dum_laxative, Group))
with(meta_liver, chisq.test(dum_antibiotics, Group))
...
for(taxon in colnames(ftbl_rclr)) {
  tryCatch({
    print(taxon)
    m <- lm(lm_data[,taxon] ~ Group + ALGLEEFT + BMI + GESLACHT + trans_roke_1 + dum_ppi + dum_laxative + dum_antibiotics, data=lm_data[,c(taxon,"Group", "ALGLEEFT", "BMI", "GESLACHT", "trans_roke_1", "dum_ppi", "dum_laxative", "dum_antibiotics")])
...
# This is caused by to few samples in e.g. trans_roke_1 variable
...
for(taxon in colnames(ftbl_rclr_PA)) {
  tryCatch({
    print(taxon)
    m <- glm(glm_data[,taxon] ~ Group + ALGLEEFT + BMI + GESLACHT + trans_roke_1 + dum_ppi + dum_laxative + dum_antibiotics, data=glm_data[,c(taxon,"Group", "ALGLEEFT", "BMI", "GESLACHT", "trans_roke_1", "dum_ppi", "dum_laxative", "dum_antibiotics")], family = binomial)
    mm <- coef(summary(m))[2,c(1,2,4)]
    names(mm)[3] <- "p_val"
...
#m <- lm(glm_data[,taxon] ~ Group + ALGLEEFT, data=glm_data[,c(taxon,"Group", "ALGLEEFT")])
...
meta_renal <- CDmeta_All %>%
  select(ID, TypeMoreThan12MonthsDag3, ALGLEEFT, BMI, GESLACHT, trans_roke_1, dum_ppi, dum_laxative, dum_antibiotics) %>%
  filter(TypeMoreThan12MonthsDag3 %in% c("Healthy Control","RTR")) %>%
  mutate(Group=relevel(factor(TypeMoreThan12MonthsDag3), ref="Healthy Control")) %>%
  select(-TypeMoreThan12MonthsDag3)


summary(meta_renal$Group)
sum(is.na(meta_renal$ALGLEEFT))
sum(is.na(meta_renal$BMI))
sum(is.na(meta_renal$GESLACHT))
sum(is.na(meta_renal$trans_roke_1))
summary(meta_renal$trans_roke_1)
sum(is.na(meta_renal$dum_ppi))
sum(is.na(meta_renal$dum_laxative))
sum(is.na(meta_renal$dum_antibiotics))

with(meta_renal, tapply(trans_roke_1, Group, summary))
with(meta_renal, tapply(dum_ppi, Group, summary))
with(meta_renal, tapply(dum_laxative, Group, summary))
with(meta_renal, tapply(dum_antibiotics, Group, summary))

wilcox.test(ALGLEEFT ~ Group, data=meta_renal)
wilcox.test(BMI ~ Group, data=meta_renal)
with(meta_renal, chisq.test(GESLACHT, Group))
with(meta_renal, chisq.test(trans_roke_1, Group))
with(meta_renal, chisq.test(dum_ppi, Group))
with(meta_renal, chisq.test(dum_laxative, Group))
with(meta_renal, chisq.test(dum_antibiotics, Group))
...
meta_renal[is.na(meta_renal$trans_roke_1),]$trans_roke_1 <- "Nee"
...
#This is for cross-sectional (ESLD and ESRD) analysis plus covariates
meta_liver <- CDmeta_All %>%
  select(ID, TypePreTx_DAG3, ALGLEEFT, BMI, GESLACHT, trans_roke_1, dum_ppi, dum_laxative, dum_antibiotics) %>%
...
summary(meta_liver$ALGLEEFT ~ meta_liver$Group)
sum(is.na(meta_liver$BMI))
sum(is.na(meta_liver$GESLACHT))
...
tapply(meta_liver$trans_roke_1, meta_liver$Group, summary)
...
meta_liver$ID <- NULL
meta_liver$GESLACHT <- as.numeric(meta_liver$GESLACHT)
meta_liver$trans_roke_1 <- as.numeric(meta_liver$trans_roke_1)
meta_liver$dum_ppi <- as.numeric(meta_liver$dum_ppi)
meta_liver$dum_laxative <- as.numeric(meta_liver$dum_laxative)
meta_liver$dum_antibiotics <- as.numeric(meta_liver$dum_antibiotics)
...
with(meta_renal, chisq.test(dum_laxative, Group))
with(meta_renal, chisq.test(dum_antibiotics, Group))
#meta_renal[is.na(meta_liver$trans_roke_1),]$trans_roke_1 <- "Nee"
...
ADONIS_Within_LTxOld <- subset(CDmeta, TypeMoreThan12Months =="LTR")
...
setwd("I:/Metagenomic sequencing/Analyzes/TxN_microbobiome analysis V1/Tx_N_merged_data/ADONIS TransplantLines")
ADONIS_Within_NTxOld <- subset(CDmeta, TypeMoreThan12Months =="RTR")
...
CDmeta_All <- CDmeta_All %>% 
  mutate(TypeMoreThan12MonthsDag3=factor(TypeMoreThan12MonthsDag3, levels=c("Healthy Control","LTR","RTR","")),
         exclude_liver_long=paste0(Timepoint_All, "_", TypeTxN_TxL_TxD_DAG3))
#5.3      Ordination Deicode----
#5.4      Fig 4A Ordination Taxa----
centroids_fig2b <- 
  read.csv("J:\\Metagenomic sequencing\\Analyzes\\TxN_microbobiome analysis V1\\Tx_N_merged_data\\Longitudinal analysis 13-12-2020\\codedata\\CDtaxa_All_sample_loadings.csv", row.names=1) %>% 
  #read.csv("~/Documents/UMGC/transplant_study/all_data/output/all_samples/CDpwys_All/n2/CDpwys_All_sample_loadings.csv", row.names=1) %>% 
  #read.csv("~/Documents/UMGC/transplant_study/all_data/output/all_samples/CDcard_All/n2/CDcard_All_sample_loadings.csv", row.names=1) %>% 
...
#5.5      Fig 4B Ordination Pathways----
centroids_fig2b <- 
...
#5.6      Fig 4C Log Ratio Plot Taxa---- 
#Taxa
ftbl_rclr <- 
