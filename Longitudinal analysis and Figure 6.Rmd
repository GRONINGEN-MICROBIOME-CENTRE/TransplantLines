...
centroids_fig6A <- 
  read.csv("~/TransplantLines/decoide_output/species/decoide_taxa_sample_loadings.csv", row.names=1) %>% 
  rownames_to_column("ID") %>% 
  select(ID, PC1, PC2) %>% 
...
centroids_figS7 <- 
  read.csv("~/TransplantLines/decoide_output/pathways/decoide_pwys_sample_loadings.csv", row.names=1) %>% 
  rownames_to_column("ID") %>% 
  select(ID, PC1, PC2) %>% 
  filter(ID %in% metadata[metadata$Timepoint_All %in% c("Healthy Control","3M","6M","12M","24M","PreTxRTR","PreTxLTR"),]$ID) %>%
  left_join(select(metadata, ID, Timepoint_All), by="ID") %>%
...
ftbl_rclr <- 
  read.csv("~/TransplantLines/mockdata/decoide_output/Mocks_Taxa_Filtered_rclr.csv", row.names=1)
...
ftbl_rclr <- 
  read.csv("~/TransplantLines/mockdata/decoide_output/Mocks_Pathways_Filtered_rclr.csv", row.names=1)
